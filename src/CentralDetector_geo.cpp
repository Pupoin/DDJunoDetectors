...
static Ref_t create_detector(Detector &lcdd, xml_h e, SensitiveDetector sens)
{
  ...
  // for(xml_coll_t xs(x_det, _Unicode(ellipsoid)); xs;++xs){
  //   cout << __LINE__ << " ellipsoid " << xs.size() << endl;
  //   xml_comp_t xc = xs;
  //   cout <<__LINE__ << " "<<  xc.x() << " " << xc.zmin() << endl;
  // }

  /// get the minimum(rmin) and maximum(rmax) of all spheres
  double totRmin = 999999 * dd4hep::m, totRmax = 0 * dd4hep::m;
  Material totSphereMat;
  for (xml_coll_t xlayer(x_det, _U(layer)); xlayer; ++xlayer)
  {
    for (xml_coll_t xc(xlayer, _U(sphere)); xc; ++xc)
    {
      ...
      if(x_sphere.nameStr() == "Steel")
        totSphereMat = lcdd.material(x_sphere.materialStr());
  ...
  Volume totSphereVol(det_name + "_Steel", totSphere, totSphereMat);
